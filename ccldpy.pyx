...
def Simulate_Rupture_Surface(eqn, eqType, region, em, elon, elat, hypd, Category,
    ...
    Fault scaling model depends on earthquake type. Originially coded in Fortran by Chiou
    and Youngs (2008; Appendix B).
    ...
    for d in np.arange(125,325,25):
        for ia in range(24):
            ...
            psx[nps] = d * sin(ang)
            psy[nps] = d * cos(ang)     
    psx = psx[:nps]; psy = psy[:nps]
    
    # Initilize variable arrays and pandas DataFrames --------------------------------------
    area = np.empty(nsim); fd = np.empty(nsim); fl = np.empty(nsim); ar = np.empty(nsim); 
    frake = np.empty(nsim); fstrike = np.empty(nsim); fdip = np.empty(nsim); rw = np.empty(nsim); 
    rl = np.empty(nsim); topd = np.empty(nsim); botd = np.empty(nsim); 
    rpx = np.empty((nsim,4,2)); rpy = np.empty((nsim,4,2)); rpz = np.empty((nsim,4,2)); 
    ckseisd = np.empty(nsim); clsd = np.empty(nsim); jbd = np.empty(nsim);  cld = np.empty(nsim);
    ffx = np.empty(5); ffy = np.empty(5); ffz = np.empty(5); p_clst = np.empty(3);
    #fflon = np.empty(5); #fflat = np.empty(5); 
    dminr = np.empty((nps,nsim)); dminjbr = np.empty((nps,nsim)); pt = np.empty((3,4)); 
    medrd = np.empty(nps); medjbd = np.empty(nps); msim = np.empty(nps); jbsim = np.empty(nps)
    sigrd = np.zeros(nps); averd = np.zeros(nps) # keep as zeros
    ...
    # Check if model is defined, otherwise rever to defaults:
    if model == None:
        if eqType == 'crustal':
            model = 'leonard_2014'
        elif eqType == 'stable':
    ...
    for isim in range(nsim):
        ...
        if eqType == 'crustal':
            ...
            if model == 'wells_and_coppersmith_1994':
                a = -3.49; b = 0.91
                area[isim] = 10.0**(a + b*em + 0.24*gasdev(idum))
            # Leonard (2014):
            elif model == 'leonard_2014':
                # Using Length and Width Relationships (consistent AR and A):
                if mech == 'SS':
                    # Compute Length:
                    a = 4.16; b = 1.667; s = 0.390
                    simfault_length = 10**( (em - a - s*gasdev(idum))/b )
                    # Compute Width:
                    a = 3.885; b = 2.50; s = 0.065
                    simfault_width = 10**( (em - a - s*gasdev(idum))/b )
                    # Compute Area:
                    area[isim] = simfault_length * simfault_width
                else:
                    # Compute Length:
                    a = 4.27; b = 1.667; s = 0.460
                    simfault_length = 10**( (em - a - s*gasdev(idum))/b )
                    # Compute Width:
                    a = 3.670; b = 2.50; s = 0.060
                    simfault_width = 10**( (em - a - s*gasdev(idum))/b )
                    # Compute Area:
                    area[isim] = simfault_length * simfault_width
            else:
                print('ERROR: invalid model (' + model + ') for eqType "' + eqType + '".')
                return
                
        elif eqType == 'interface':
            # Contreras et al. (2022):
            if model == 'contreras_et_al_2022':
                a = -3.8290; b = 1.0
            ...
            # Leonard (2014):
            if model == 'leonard_2014':
                # Using Length and Width Relationships (consistent AR and A):
                if mech == 'SS':
                    # Compute Length:
                    a = 4.16; b = 1.667; s = 0.090
                    simfault_length = 10**( (em - a - s*gasdev(idum))/b )
                    # Compute Width:
                    a = 4.200; b = 2.50; s = 0.030
                    simfault_width = 10**( (em - a - s*gasdev(idum))/b )
                    # Compute Area:
                    area[isim] = simfault_length * simfault_width
                else:
                    # Compute Length:
                    a = 4.315; b = 1.667; s = 0.195
                    simfault_length = 10**( (em - a - s*gasdev(idum))/b )
                    # Compute Width:
                    a = 4.125; b = 2.50; s = 0.045
                    simfault_width = 10**( (em - a - s*gasdev(idum))/b )
                    # Compute Area:
                    area[isim] = simfault_length * simfault_width
            else:
        ...
        if (eqType == 'crustal'):
            ...
            # Using Length and Width Relationships from before (consistent AR and A):
                ar[isim] = simfault_length / simfault_width
            else:
                print('ERROR: invalid model (' + model + ') for eqType "' + eqType + '".')
                return
        ...
        rpx[isim][3][0] = xf; rpx[isim][3][1] = xb; rpy[isim][3][0] = yf
        rpy[isim][3][1] = yb; rpz[isim][3][0] = hypd; rpz[isim][3][1] = hypd
        
        # Compute closest distance for each pseudo-site ....................................
        for i_s in range(nps):

            # load data into pt array for rupture distances:
            pt[0][0] = rpx[isim][0][1] - psx[i_s]; pt[1][0] = rpy[isim][0][1] - psy[i_s]; 
            pt[2][0] = rpz[isim][0][1] 
            pt[0][1] = rpx[isim][0][0] - psx[i_s]; pt[1][1] = rpy[isim][0][0] - psy[i_s]; 
            pt[2][1] = rpz[isim][0][0] 
            pt[0][2] = rpx[isim][1][0] - psx[i_s]; pt[1][2] = rpy[isim][1][0] - psy[i_s]; 
            pt[2][2] = rpz[isim][1][0] 
            pt[0][3] = rpx[isim][1][1] - psx[i_s]; pt[1][3] = rpy[isim][1][1] - psy[i_s]; 
            pt[2][3] = rpz[isim][1][1] 

            dminr[i_s][isim], p_clst_r = DEMDT(pt)

            # load data into pt array for JB distance:
            pt[2][0] = 0.0; pt[2][1] = 0.0; pt[2][2] = 0.0; pt[2][3] = 0.0
    ...
    topd_mx = max(topd); topd_mn = min(topd); botd_mx = max(botd); botd_mn = min(botd)
   
    # Convert selected FF xy to LL using haversine formulation -----------------------------
    nffpts=5

    ffx[0]=rpx[ksim][0][1]; ffy[0]=rpy[ksim][0][1]; ffz[0]=rpz[ksim][0][1]
    ffx[1]=rpx[ksim][0][0]; ffy[1]=rpy[ksim][0][0]; ffz[1]=rpz[ksim][0][0]
    ffx[2]=rpx[ksim][1][0]; ffy[2]=rpy[ksim][1][0]; ffz[2]=rpz[ksim][1][0]
    ffx[3]=rpx[ksim][1][1]; ffy[3]=rpy[ksim][1][1]; ffz[3]=rpz[ksim][1][1]
    ffx[4]=rpx[ksim][0][1]; ffy[4]=rpy[ksim][0][1]; ffz[4]=rpz[ksim][0][1]
...
def DEMDT(P):
    ...
    E0[0] = P[0][1] - B[0]; E0[1] = P[1][1] - B[1]; E0[2] = P[2][1] - B[2]
    E1[0] = P[0][3] - B[0]; E1[1] = P[1][3] - B[1]; E1[2] = P[2][3] - B[2]
    ...
    if dis4 < dclst:
        ...
        dclst = dis4  
    dclst = sqrt(dclst)
    P_clst[0] = E_clst[0][iclst]; P_clst[1] = E_clst[1][iclst]; P_clst[2] = E_clst[2][iclst]
